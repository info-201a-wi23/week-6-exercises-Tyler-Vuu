---
title: "The Office Report but mine"
author: "Tyler"
date: '2023'
output:
...
rating_per_season <- office_df %>% group_by(Season) %>% summarize(avg_ratings = max(mean(Rating,na.rm = TRUE)))
...
ggplot(data = office_df)+
  geom_point(mapping = aes(x = Season, y = Rating))
```

```{r, echo=FALSE}
# Find the highest IMDB rating for any episode of The Office and pull the value
highest_rating <- office_df(max(Rating, na.rm = TRUE))
